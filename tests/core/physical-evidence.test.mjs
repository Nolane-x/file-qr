...
function prep({ scenario = 'fqr2-windows-to-android', bytes = 65536, protocol = 'FQR2' } = {}) {
  return {
    schemaVersion: 1,
    scenario,
...
function completion({
  ...
  return {
    sender: {
      ...
      ...(sender === 'windows' ? { cameraDevicePresent: false, cameraDeviceCount: 0 } : { physicalDevice: true, emulatorRejected: true, cameraPermissionGranted: true, cameraOwnerObserved: false }),
    },
    receiver: {
      platform: receiver,
...
function finalFor(scenario, opts = {}) {
  const p = prep({ scenario, bytes: opts.bytes ?? 65536, protocol: scenario.startsWith('fqr1-') ? 'FQR1' : 'FQR2' });
...
test('preparation canonicalization is strict and stable', () => {
  ...
  assert.throws(
    () => validatePreparation({ ...p, bypass: true }, { authoritative: true }),
    /FQR_EVIDENCE_UNKNOWN_KEY/,
  );
  assert.throws(
    () => validatePreparation({ ...p, control: { ...p.control, commitSha: 'ABC' } }, { authoritative: true }),
    /FQR_EVIDENCE_SHA/,
  );
});
...
test('baseline Windows to Android evidence passes only with exact bytes and Android camera ownership', () => {
  ...
  assert.equal(badBytes.assertions.exactBytesMatch, false);
...
test('Android to Windows evidence requires Windows camera presence and physical-use observation', () => {
  const p = prep({ scenario: 'fqr2-android-to-windows' });
  const good = completion({ sender: 'android', receiver: 'windows' });
  assert.equal(finalizeEvidence(p, good, { authoritative: true }).result, 'PASS');
...
test('special FQR2 scenario boundaries are exact', () => {
  ...
  assert.equal(finalFor('fqr2-mid-cycle', { joinedMidCycle: false }).result, 'FAIL');
  ...
  assert.equal(finalFor('fqr2-interruption-resume', { interruptionPerformed: true, resumeObserved: false }).result, 'FAIL');
});
...
test('FQR1 compatibility stays separate and receiver hardware rules still apply', () => {
  ...
  const wrong = prep({ scenario: 'fqr1-windows-to-android', protocol: 'FQR2' });
  assert.throws(() => validatePreparation(wrong, { authoritative: true }), /FQR_EVIDENCE_PROTOCOL/);
});
...
test('privacy and unknown completion fields fail closed', () => {
  ...
  assert.equal(finalizeEvidence(p, leaked, { authoritative: true }).result, 'FAIL');

  const injected = completion();
  injected.guaranteedThroughputMbps = 500;
  assert.throws(() => finalizeEvidence(p, injected, { authoritative: true }), /FQR_EVIDENCE_UNKNOWN_KEY/);
});
...
test('timestamp ordering and final-record authority are validated', () => {
  ...
  assert.throws(() => finalizeEvidence(p, c, { authoritative: true }), /FQR_EVIDENCE_TIME/);
...
test('matrix requires exact build lineage and every Issue #50 category', () => {
  ...
  mixed[1].build.workflowRunId = 124;
