...
function updateProgress(done, total, label) {
  ...
  ui.rate.textContent = formatRate(done, elapsedMs);
...
function stopConnectionTimer() {
  if (current.connectionTimer) clearTimeout(current.connectionTimer);
  current.connectionTimer = null;
}

function failTransfer(detail) {
...
function startConnectionTimer() {
  stopConnectionTimer();
  current.connectionTimer = setTimeout(() => {
    if (current.state !== 'connecting') return;
    failTransfer('Direct connection timed out after 30 seconds. Retry or use QR Stream in the native app.');
...
function attachConnectionDiagnostics(peer) {
  peer.addEventListener('connectionstatechange', () => {
    ...
    if (state === 'failed') {
      failTransfer('The direct WebRTC path failed. Retry on another network or use the native QR Stream.');
      return;
    }
    if (state === 'closed' && LIVE_CONNECTION_STATES.has(current.state)) {
      failTransfer('The direct WebRTC connection closed before the transfer finished.');
    }
  });
}
...
async function sendFile(file) {
  ...
  try {
    ...
    attachConnectionDiagnostics(peer);
    ...
    channel.addEventListener('open', async () => {
      ...
      try {
        ...
        failTransfer(error?.message || 'The send stream failed.');
      }
    });
    channel.addEventListener('error', () => failTransfer('The direct data channel failed. Your file stayed local.'));
  } catch (error) {
    cleanup({ keepView: true });
  ...
  try {
    ...
    peer.addEventListener('datachannel', ({ channel }) => {
      ...
      channel.addEventListener('open', () => {
        stopConnectionTimer();
        current.startedAt = performance.now();
        ui.status.textContent = 'Direct channel open. Waiting for file metadata…';
      });
      ...
      channel.addEventListener('error', () => failTransfer('The receive data channel failed before the file completed.'));
    });

    socket.addEventListener('message', (event) => handleRemoteSignal(peer, socket, event, 'receiver', candidateBuffer).catch((error) => failTransfer(error?.message || 'Signaling failed.')));
    socket.addEventListener('close', (event) => {
      if (current.state === 'connecting' && event.code === 4000) {
        cleanup({ keepView: true });
        setState('expired');
      }
    });
...
window.addEventListener('pagehide', () => { cleanup(); wakeLock.destroy(); });
